...
class WebstorageLinksController < ApplicationController
  DOWNLOAD_COUNT_LIMIT = 16
  DOWNLOAD_HOUR_LIMIT  = 5
  def destroy
    msg = ''
...
#    raise Exception.new(params)
  ...
  def show_to_link
    ...
    # 如果没有引用地址或是生产环境的引用地址的url不包含www.banshujiang.cn,那么一律跳转到详情页
    if !request.referrer || (request.referrer =~ /www\.banshujiang\.cn|ebook\.jiani\.info/i) == nil && Rails.env == 'production' then
      redirect_to @e_book
      return
    end
